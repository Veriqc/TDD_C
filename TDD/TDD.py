...
class TDD:
    ...
    def self_copy(self):
        ...
        temp.index_set = [copy.copy(k) for k in self.index_set]
...
#             the_successors[k].weight=weigs[k]=0
    ...
    weigs_abs=[abs(weig)/epi for weig in weigs]
    weig_max=weigs[weigs_abs.index(max(weigs_abs))]
    try :
        weigs=[weig/weig_max for weig in weigs]
    except  ZeroDivisionError:
        raise(ValueError(f"weig_max is {weig_max} and weigs is {weigs}"))
    for k in range(len(the_successors)):
        if get_int_key(weigs[k])==(0,0):
            node=Find_Or_Add_Unique_table(-1)
            the_successors[k]=TDD(node)
...
def conjugate(tdd):
    ...
    low.weight=low.weight.conjugate()
    high. weight=high.weight.conjugate()
    res=normalize(v,[low,high])
    res.weight *= tdd.weight.conjugate()
    # todo:to avoid zero bug problem
