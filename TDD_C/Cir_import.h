...
dd::TDD apply(dd::TDD tdd, std::string nam, std::vector<dd::Index> index_set) {
	...
	if (nam == "cx") {
		...
		case 1:
			temp_tdd = dd::Matrix2TDD(dd::Nm, index_set);
			break;
		case 2:
			temp_tdd = dd::Matrix2TDD(dd::Ym, index_set);
			break;
		case 3:
			temp_tdd = dd::diag_matrix_2_TDD(dd::Zm, index_set);
			break;
		case 4:
			temp_tdd = dd::Matrix2TDD(dd::Hm, index_set);
			break;
		case 5:
			temp_tdd = dd::diag_matrix_2_TDD(dd::Sm, index_set);
			break;
		case 6:
			temp_tdd = dd::diag_matrix_2_TDD(dd::Sdgm, index_set);
			break;
		case 7:
			temp_tdd = dd::diag_matrix_2_TDD(dd::Tm, index_set);
			break;
		case 8:
		...
		default:
			if (nam[0] == 'r' and nam[1] == 'z') {
				...
				dd::DD_matrix Rzmat = { {{ 1, 0 }, { 0, 0 } }, {{ 0, 0 }, { cos(theta), sin(theta) } } };
...
int* Simulate_with_partition1(std::string path, std::string  file_name) {
	...
	for (int k = 0; k < par.size(); k++) {
		...
		for (int k1 = 0; k1 < par[k][1].size(); k1++) {
			...
			string nam = gate_set[gate_idx].name;
			temp_tdd2 = apply(temp_tdd2, nam, Index_set[gate_idx]);
		}
