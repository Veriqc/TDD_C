...
xt::xarray<dd::ComplexValue> eyeMat(int n) {
    if (n == 1) {
        return xgate::Imat;
    }

    xt::xarray<dd::ComplexValue> I = eyeMat(n - 1);
...
xt::xarray<dd::ComplexValue> cM(xt::xarray<dd::ComplexValue> mat, int c) {
    if (c == 0) return mat;
    ...
    xt::view(combined, 0, 0, xt::all(), xt::all()) = eyeMat(mat.dimension() / 2); // Place identity matrix
...
int main() {
    ...
    auto cx = cM(xgate::Phasemat(1.5),1);
