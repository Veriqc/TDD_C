...
namespace cir_tn {
	...
	std::map<int, std::map<int, std::vector<int>>>  cir_partition1(std::map<int, gate> gate_set, int cx_cut_max) {
	...
	std::map<int, std::map<int, std::vector<int>>>  cir_partition2(std::map<int, gate> gate_set, int cx_cut_max, int c_part_width) {
		std::map<int, std::map<int, std::vector<int>>>   par;
		int cx_cut = 0;
		int block = 0;
	...
	dd::TensorNetwork cir_2_tn(std::string path, std::string  file_name, std::unique_ptr<dd::Package<>>& ddpack) {
			...
			ddpack->varOrder = get_var_order();
			std::map<int, std::vector<dd::Index>> Index_set = get_index(gate_set, ddpack->varOrder);

			dd::TensorNetwork tn;

			for (int k = 0; k < gate_set.size(); k++) {
				tn.tensors.push_back(gate_2_tensor(gate_set[k].name, Index_set[k]));
			}
